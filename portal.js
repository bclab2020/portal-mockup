...
function renderSidebarExperts() {
    const sidebarContainer = document.getElementById('sidebar-experts-list');
    if (!sidebarContainer) return;
    
    sidebarContainer.innerHTML = '';
    
    const experts = articlesDb.experts || [];
    const displayExperts = experts.slice(0, 3);
    
    displayExperts.forEach((exp, idx) => {
        const isLast = (idx === displayExperts.length - 1);
        const borderStyle = isLast ? '' : 'border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 10px;';
        
        let avatarHtml = '';
        if (exp.img_id && (exp.img_id.startsWith('images/') || exp.img_id.startsWith('http://') || exp.img_id.startsWith('https://'))) {
            avatarHtml = `<div class="composer-avatar" style="background-image: url('${exp.img_id}'); background-size: cover; background-position: center; border-radius: 50%; width: 40px; height: 40px; flex-shrink: 0;"></div>`;
        } else {
            const nameAbbrev = exp.name ? exp.name.substring(0, 2) : '専門';
            avatarHtml = `<div class="composer-avatar" style="background: linear-gradient(45deg, var(--accent-orange), #ff5722); display: flex; align-items: center; justify-content: center; font-weight: 700; color: #fff; border-radius: 50%; width: 40px; height: 40px; flex-shrink: 0; font-size: 11px;">${nameAbbrev}</div>`;
        }
        ...
        const expertHtml = `
            <div style="display: flex; align-items: center; gap: 10px; ${borderStyle}">
                ...
                <div style="min-width: 0; flex: 1;">
                    <div style="font-weight: 700; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${exp.name} (${exp.field || '専門家'})</div>
                    <div style="color: var(--text-secondary); text-overflow: ellipsis; overflow: hidden; white-space: nowrap; font-size: 11px;">${exp.title || ''}</div>
                </div>
            </div>
        `;
        sidebarContainer.insertAdjacentHTML('beforeend', expertHtml);
    });
    
    const linkHtml = `
        <div style="margin-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 10px; text-align: center;">
            <a href="experts.html" style="text-decoration: none; color: var(--accent-orange); font-weight: 700; display: inline-flex; align-items: center; gap: 5px; font-size: 12px; transition: opacity 0.2s;" onmouseover="this.style.opacity=0.8" onmouseout="this.style.opacity=1">
                専門家一覧を見る ➔
            </a>
        </div>
    `;
    sidebarContainer.insertAdjacentHTML('beforeend', linkHtml);
}
function getAllArticlesForCategory(category) {
    let list = [];
...
function parseMarkdown(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].startsWith('* ')) {
            let content = lines[i].substring(2);
            ...
            if (lines[i].trim() !== '' && !lines[i].startsWith('<h') && !lines[i].startsWith('<u') && !lines[i].startsWith('<l')) {
                lines[i] = '<p>' + lines[i] + '</p>';
            }
        }
    }
...
window.openArticleDetail = function(category, index, event) {
    if (event) {
        event.stopPropagation();
        event.preventDefault();
    }
    ...
    let checkListHtml = '';
    if (art.check_list && art.check_list.length > 0) {
        checkListHtml = `
            <div class="checklist-container">
                <div class="checklist-title">
                    <span>📋 セルフチェック診断リスト</span>
                </div>
                ${art.check_list.map((item, idx) => `
                    <label class="checklist-item" onclick="event.stopPropagation()">
                ...
                <div style="margin-top: 15px; font-size: 11px; color: var(--text-secondary);">
                    ※気になる項目がある方は、下のボタンを押してアライメントを測定しましょう。
                </div>
            </div>
        `;
    }
    ...
    modalBody.innerHTML = `
        <div class="modal-body">
            <h1 style="font-size: 1.6rem; font-weight: 700; margin-bottom: 10px; color: var(--text-primary); line-height: 1.4;">${art.title}</h1>
            <div style="display: flex; gap: 10px; font-size: 12px; color: var(--text-secondary); margin-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 10px;">
                <span>${art.time || ''}</span>
                <span>${art.author || ''}</span>
            </div>
            ...
            ${art.img_id ? `
                <div style="width: 100%; height: 260px; border-radius: 12px; overflow: hidden; margin-bottom: 20px;">
                    <img src="${art.img_id.startsWith('photo-') ? 'https://images.unsplash.com/' + art.img_id + '?w=800&auto=format&fit=crop&q=80' : art.img_id}" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
            ` : ''}
            
            <div style="font-size: 1.05rem; line-height: 1.7; color: var(--text-primary); margin-bottom: 20px; font-weight: 500;">
                ${art.p || ''}
            </div>
            
            <div style="border-top: 1px solid rgba(255,255,255,0.05); padding-top: 15px;">
                ${parseMarkdown(art.detail_p)}
            </div>
    ...
    // Delay adding active class for animation transition
    setTimeout(() => {
